"""
V8 Parallel Coordinates: single combined plot, actual tick values only.
Height auto-resizes. Large bold fonts. Extra title margin.
"""
...
def plot_parallel_coords_v8(data: pd.DataFrame, metric_column: str,
    ...
    fig = go.Figure(
        data=go.Parcoords(
            line=dict(
                ...
                colorscale='RdYlGn_r',
                showscale=True,
                colorbar=dict(title=dict(text=metric_column, font=dict(size=16))),
            ),
            labelfont=dict(size=17, color='black'),
    ...
    fig.update_layout(
        ...
        margin=dict(l=100, r=60, t=110, b=40),
    )
