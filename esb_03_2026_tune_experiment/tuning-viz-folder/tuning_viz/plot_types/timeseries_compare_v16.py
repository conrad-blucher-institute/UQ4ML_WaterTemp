"""
V16 Time Series Comparison: model picker + triple stacked Plotly charts.

Changes from v15:
- Training trace overlaid on the validation chart
- New Testing chart between Validation and 2021
- Toggle buttons to show/hide Training, Validation, and 2021 charts
- Hide Legends button
- Metric dropdown selector in the left panel (like explorer)
- Resizable left panel (drag handle)
...
def plot_timeseries_compare_v16(predictions_dir: str, output_dir: str) -> Optional[str]:
    ...
    # Build metric <option> tags for the dropdown
    metric_options_html = ''
    for mc in metric_cols:
    ...
    html = f"""<!DOCTYPE html>
<html><head>
<title>Time Series Comparison (v16)</title>
    ...
    <div class="table-wrap">
        <table>
            <thead><tr>
                ...
                <th id="metricTh1" onclick="sortTable('__metric1__')">val_mae</th>
                <th id="metricTh2" onclick="sortTable('__metric2__')">mae_2021</th>
            </tr></thead>
...
// Which metric columns to display in the table
var displayMetric1 = metricCols[0] || 'val_mae';
var displayMetric2 = metricCols.length > 1 ? metricCols[1] : null;

// Chart visibility
var chartVisible = {{ train: true, val: true, test: true, '2021': true }};
...
function recalcCharts() {{
    if (selectedModels.length > 0) {{
        ['chartTrain','chartVal','chartTest','chart2021'].forEach(function(id) {{
...
function changeMetric() {{
    ...
    displayMetric1 = chosen;
    // Find a second metric that is different from the first
    displayMetric2 = null;
    for (var i = 0; i < metricCols.length; i++) {{
        if (metricCols[i] !== chosen) {{ displayMetric2 = metricCols[i]; break; }}
    }}
    // Update table headers
    var th1 = document.getElementById('metricTh1');
    th1.textContent = displayMetric1;
    th1.setAttribute('onclick', "sortTable('" + displayMetric1 + "')");
    var th2 = document.getElementById('metricTh2');
    if (displayMetric2) {{
        th2.textContent = displayMetric2;
        th2.setAttribute('onclick', "sortTable('" + displayMetric2 + "')");
        th2.style.display = '';
    }} else {{
        th2.style.display = 'none';
    }}
    sortCol = displayMetric1;
    sortAsc = true;
    applyFilters();
}}
...
function toggleChart(which) {{
    ...
    var btnId = which === '2021' ? 'toggle2021' : 'toggle' + which.charAt(0).toUpperCase() + which.slice(1);
...
function renderTable() {{
    ...
    filteredData.forEach(function(m) {{
        ...
        var m1val = m[displayMetric1];
        var m2val = displayMetric2 ? m[displayMetric2] : null;
        tr.innerHTML =
            ...
            '<td>' + m.leadtime + 'h</td>' +
            '<td>' + m.cycle + '</td>' +
            '<td>' + (m1val != null ? m1val.toFixed(4) : '-') + '</td>' +
            (displayMetric2 ? '<td>' + (m2val != null ? m2val.toFixed(4) : '-') + '</td>' : '');
...
// Initialize metric columns in table header
changeMetric();
