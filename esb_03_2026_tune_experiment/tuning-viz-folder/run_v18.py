"""
Run V18 visualizations for tuning results.

Usage:
    python run_v18.py                       # GUI folder picker
    python run_v18.py --test-iterations 3   # duplicate data 3x for testing iteration plot

V18 changes:
    - Time Series: Loss Curves chart (loss vs val_loss per epoch) with toggle button,
      loaded from _history.json in keras_files/ sibling directory
    - Carries forward v17 features: metric checkboxes, metric values in traces,
      all-metrics hover in explorer
"""
...
from tuning_viz.plot_types.unified_plot_v16 import plot_unified_v16
...
from tuning_viz.plot_types.timeseries_compare_v18 import plot_timeseries_compare_v18
...
def run_v18(data, metric_column, output_dir, results_folder=None):
    print(f"\n{'='*60}")
    print(f"V18 Visualizations: {output_dir}")
    print(f"{'='*60}")
    ...
    print("\n--- Unified Explorer (All Metrics in Hover + Send to TS) ---")
    plot_unified_v16(data, metric_column, output_dir=output_dir)
    ...
    if results_folder:
        ...
        if Path(pred_dir).exists() and (Path(pred_dir) / '_index.csv').exists():
            print("\n--- Time Series Comparison (v18 + Loss Curves) ---")
            plot_timeseries_compare_v18(pred_dir, output_dir=output_dir)
        else:
            print("\n--- Time Series Comparison: SKIPPED (no predictions/ folder, run generate_predictions.py first) ---")

    print(f"\n{'='*60}")
    print(f"All v18 plots saved to: {output_dir}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run V18 tuning visualizations")
    parser.add_argument(
    ...
    for csv_path in csv_paths:
        ...
        run_v18(data, metric_column, output_dir, results_folder=str(folder))
