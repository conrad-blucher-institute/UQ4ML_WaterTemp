"""
V8 Unified Plot: Scatter / Boxplot / Violin in one HTML.
- Toggle between plot types via 3 radio-style buttons
- Y-axis metric dropdown (works for all 3 plot types)
- Dual mode: side-by-side with two independently selectable Y metrics
...
def plot_unified_v8(data: pd.DataFrame, metric_column: str, output_dir: Optional[str] = None) -> Optional[str]:
    ...
    # For the right Y-axis, default to second metric if available
    y_axis_options_right = ''
    for i, mc in enumerate(metric_cols):
        sel = ' selected' if i == 1 else ''
        y_axis_options_right += f'<option value="{mc}"{sel}>{mc}</option>\n'
...
<div class="controls">
    ...
    <div class="filter-bar">
        ...
        <div class="filter-group" id="yRightGroup" style="display:none;">
            <label>Y Right:</label>
            <select id="yAxisRight" onchange="rebuild()">
                {y_axis_options_right}
            </select>
        </div>
...
function setPlotType(type) {{
    ...
    // Show points button only for box/violin
...
function toggleDual() {{
    ...
    document.getElementById('yRightGroup').style.display = dualMode ? '' : 'none';
...
function rebuild() {{
    ...
    if (dualMode) {{
        var yMetricR = document.getElementById('yAxisRight').value;
        var filteredR = paramFiltered.filter(function(d) {{ return d[yMetricR] != null; }});
        var resultR = buildTraces(filteredR, yMetricR, xParam, colorBy);
        var layoutR = buildLayout(yMetricR, xParam, colorBy, resultR.useColor, filteredR);
