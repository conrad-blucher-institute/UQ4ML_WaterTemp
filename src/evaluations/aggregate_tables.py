...
def aggregateTable(leadTimes, cycles, architectures, threshold, byCycle, URI=False, padding = 24):
    ...
    if byCycle == False:
        ...
        pre_aggregate_byUQMethod_file(leadTimes, cycles, architectures)
            ...
            for architecture in architectures:
                ...
                for model in model_list:
                    ...
                    if byCycle == True:
                        ...
                        df = pd.read_csv(str(leadTime) + 'h_' + architecture + '_Cycle_' + str(cycle) + '_Model_' + model + ".csv")
                        
                        saveIdentifier = "byCycle"
                        
                    else:
                        
                        df = pd.read_csv("_".join(architectures) + "_byUQMethod_UQ_Files/" + str(leadTime) + 'h_' + architecture + '_Model_' + model + ".csv")
    ...
    if URI == True:
        ...
        # Places into csvs
        results_URI_df.to_csv('URI_metrics_results_performance_' + saveIdentifier + 'AggregateTable.csv')
        results_URI_padded_df.to_csv('URI_padded_metrics_results_performance_' + saveIdentifier + 'AggregateTable.csv')
...
def pre_aggregate_byUQMethod_file(leadTimes, cycles, architectures):
        ...
        for architecture in architectures:
            ...
            for model in model_list:
                ...
                for cycle in cycles:
                    ...
                    # Retrieves Files
                    file_name = f"{leadTime}h_{architecture}_Cycle_{cycle}_Model_{model}.csv"
                ...
                mainDf.to_csv(folder_path / f"{leadTime}h_{architecture}_Model_{model}.csv")
...
if __name__ == "__main__":
    ...
    aggregateTable(leadTimes, cycles, architectures, threshold, byCycle, True ,24)
