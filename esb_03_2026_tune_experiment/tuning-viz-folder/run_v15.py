"""
Run V15 visualizations for tuning results.

Usage:
    python run_v15.py                       # GUI folder picker
    python run_v15.py --test-iterations 3   # duplicate data 3x for testing iteration plot

V15 changes:
    - Explorer: fixed Send to Time Series URL (uses same origin)
    - Time Series: air temp trace, multi-select checkbox filters, Show Selected toggle
"""
...
from tuning_viz.plot_types.timeseries_compare_v15 import plot_timeseries_compare_v15
...
def run_v15(data, metric_column, output_dir, results_folder=None):
    print(f"\n{'='*60}")
    print(f"V15 Visualizations: {output_dir}")
    print(f"{'='*60}")
    ...
    if results_folder:
        ...
        if Path(pred_dir).exists() and (Path(pred_dir) / '_index.csv').exists():
            ...
            plot_timeseries_compare_v15(pred_dir, output_dir=output_dir)
        else:
            print("\n--- Time Series Comparison: SKIPPED (no predictions/ folder, run generate_predictions.py first) ---")

    print(f"\n{'='*60}")
    print(f"All v15 plots saved to: {output_dir}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run V15 tuning visualizations")
    parser.add_argument(
    ...
    for csv_path in csv_paths:
        ...
        run_v15(data, metric_column, output_dir, results_folder=str(folder))
