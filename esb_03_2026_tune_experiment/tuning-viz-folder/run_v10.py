"""
Run V9 visualizations for tuning results.

Usage:
    python run_v10.py                       # GUI folder picker
    python run_v10.py --test-iterations 3   # duplicate data 3x for testing iteration plot
...
V9 changes from V8:
    - Unified explorer (scatter/box/violin toggle, dual Y-axis mode)
      replaces separate scatter, boxplot, and violin HTMLs
"""
...
from tuning_viz.plot_types.unified_plot_v10 import plot_unified_v10
...
def run_v10(data, metric_column, output_dir):
    """Generate all v10 plots."""
    print(f"\n{'='*60}")
    print(f"V9 Visualizations: {output_dir}")
    print(f"{'='*60}")
    ...
    print("\n--- Unified Explorer (scatter/box/violin + dual Y) ---")
    plot_unified_v10(data, metric_column, output_dir=output_dir)
    ...
    print(f"\n{'='*60}")
    print(f"All v10 plots saved to: {output_dir}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run V9 tuning visualizations")
    parser.add_argument(
    ...
    for csv_path in csv_paths:
        ...
        run_v10(data, metric_column, output_dir)
