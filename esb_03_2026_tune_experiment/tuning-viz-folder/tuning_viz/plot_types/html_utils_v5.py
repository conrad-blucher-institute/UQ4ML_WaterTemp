"""
Shared HTML utilities for v5 plots.
Provides cycle dropdown switching and trace-level combined filtering.
"""


def wrap_figures_with_cycle_dropdown(figure_htmls: dict, page_title: str) -> str:
    """
    Wrap multiple plotly figures (one per cycle) in a single HTML page
    with a dropdown to switch between them.

    Args:
        figure_htmls: dict of {label: plotly_html_string}
                      e.g. {"All Cycles": fig.to_html(...), "Cycle 1": fig.to_html(...)}
        page_title: page title shown at top
    """
    options_html = ""
    divs_html = ""
...
function switchCycle() {{
    ...
    // Trigger resize so plotly redraws correctly
