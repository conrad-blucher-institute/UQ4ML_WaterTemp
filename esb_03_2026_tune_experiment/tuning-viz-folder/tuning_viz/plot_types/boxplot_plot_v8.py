"""
V8 Boxplot: x-axis selector, color-by, filter dropdowns, Models(N).
Points offset to the right of box. Y-axis starts at 0. Auto-height.
"""
...
def plot_boxplot_v8(data: pd.DataFrame, metric_column: str, output_dir: Optional[str] = None) -> Optional[str]:
...
function rebuild() {{
    ...
    var traces = [];
    var pointsMode = showJitter ? 'all' : false;
    ...
    if (colorBy === 'none' || colorBy === xParam) {{
        ...
        colorVals.forEach(function(cv, ci) {{
            ...
            traces.push({{
                ...
                marker: {{ color: colors[ci % colors.length], size: 3, opacity: 0.5 }},
                boxpoints: pointsMode,
                jitter: 0.5, pointpos: 0.8
            }});
        }});
