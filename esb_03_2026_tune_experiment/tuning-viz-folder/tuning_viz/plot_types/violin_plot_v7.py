"""
V7 Violin: same controls as boxplot (x-axis selector, color-by, jitter, legend).
Violin shape shows full distribution density.
"""
...
def plot_violin_v7(data: pd.DataFrame, metric_column: str, output_dir: Optional[str] = None) -> Optional[str]:
    ...
    if (colorBy === 'none' || colorBy === xParam) {{
        ...
        sortedKeys.forEach(function(key, i) {{
            traces.push({{
                ...
                points: pointsMode, jitter: 0.4, pointpos: 0,
