...
class MSETuner(BaseHyperparameterTuner):
    ...
    def __init__(self, output_dir: Path, max_workers: int = 4):
        super().__init__("MSE", output_dir, max_workers)
    ...
    def _build_model(self, config: dict):
        """
        Build an MSE model with the given configuration.
        ...
        # Compile with Adam and MSE loss; record MAE as additional metric
    ...
    def _train_model(self, model, config: dict) -> float:
        ...
        # Simplified training flow similar to MAPETuner: double-call preparingData()
        try:
        ...
        prep_kwargs = {
            ...
            'model': config.get('model', config.get('model_type', 'MSE')),
            'verbose': config.get('verbose', 0)
        }

        # Preview to infer input shape
        ...
        # Build model with injected input shape
        model = self._build_model(config)

        # Get actual training data
        data = preparingData(**prep_kwargs)
        if data is None:
            raise RuntimeError('preparingData() returned None')
        ...
        if X_val is not None:
            x_val = getattr(X_val, 'values', X_val)
            y_val = getattr(y_val, 'values', y_val)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MSE Hyperparameter Tuner")
    parser.add_argument("--output", type=Path, default="./mse_tune_results",
                        help="Output directory for progress and timing logs")
    parser.add_argument("--workers", type=int, default=4,
                        help="Number of parallel workers")
    args = parser.parse_args()
    
    tuner = MSETuner(args.output, max_workers=args.workers)
    tuner.run_tuning()
