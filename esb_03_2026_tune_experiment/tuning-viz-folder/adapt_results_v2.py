"""
Adapter v2: convert coworker's experiment results structure into the tuning-viz format.
...
Coworker structure (input):
    {source}/
      {lt}h/
        {type}-{layers}_layers-{act}-{neurons}_neurons-cycle_{c}-iteration_{i}/
...
Tuning-viz structure (output):
    {source}_adapted-to-HyperView/
      ...
      keras_files/
        ...
        {TYPE}_{lt}h_cycle{c}_{act}_{layers}L_{neurons}N_run{i}_history.json
      predictions/
        {TYPE}_{lt}h_cycle{c}_{act}_{layers}L_{neurons}N_run{i}_pred.csv
        _index.csv
        air_temp.csv

Usage:
    python adapt_results_v2.py                          # GUI folder picker
    python adapt_results_v2.py --source path/to/folder  # direct path
"""
...
def parse_experiment_folder(folder_name):
    """Parse coworker folder name like '{type}-{layers}_layers-{act}-{neurons}_neurons-cycle_{c}-iteration_{i}'.

    Handles mape-, mse-, crps- prefixes.
    Returns dict with model_type, layers, activation, neurons, cycle, iteration or None.
    """
    m = re.match(
        r'^(mape|mse|crps)-(\d+)_layers-(\w+)-(\d+)_neurons-cycle_(\d+)-iteration_(\d+)$',
        folder_name,
    )
    if not m:
        return None
    return {
        'model_type': m.group(1).upper(),  # MAPE, MSE, or CRPS
        'num_layers': int(m.group(2)),
        'activation': m.group(3),
        'neurons': int(m.group(4)),
        'cycle': int(m.group(5)),
        'iteration': int(m.group(6)),
    }


def build_basename(model_type, leadtime, cycle, activation, num_layers, neurons, run_num):
    """Build the tuning-viz canonical model name."""
    return f"{model_type}_{leadtime}h_cycle{cycle}_{activation}_{num_layers}L_{neurons}N_run{run_num}"
...
def merge_predictions(experiment_dir):
    """Merge coworker's separate prediction CSVs into one unified CSV.
...
def adapt(source_dir):
    ...
    # Auto-detect model type from first experiment folder found
    detected_type = None
    for lt_dir in sorted(lt_dirs):
        for d in lt_dir.iterdir():
            if d.is_dir():
                cfg = parse_experiment_folder(d.name)
                if cfg:
                    detected_type = cfg['model_type']
                    break
        if detected_type:
            break

    if not detected_type:
        print("ERROR: Could not detect model type from experiment folder names.")
        print(f"  Expected folders matching: {{mape|mse|crps}}-{{layers}}_layers-{{act}}-{{neurons}}_neurons-cycle_{{c}}-iteration_{{i}}")
        sys.exit(1)
    ...
    for lt_dir in sorted(lt_dirs):
        ...
        for exp_dir in sorted(exp_dirs):
            ...
            basename = build_basename(
                cfg['model_type'], leadtime, cfg['cycle'], cfg['activation'],
    ...
    # Write progress CSV named after the detected type
    type_lower = detected_type.lower()
    progress_df = pd.DataFrame(progress_rows)
    progress_path = dest / f'{type_lower}_progress.csv'
    ...
    # Write timing CSV (empty, just header)
    timing_path = dest / f'{type_lower}_timing.csv'
    ...
    print(f"\n{'='*60}")
    print(f"Adapted {total_models} {detected_type} models -> {dest}")
    print(f"{'='*60}")
    print(f"\nTo visualize:")
    print(f"  python run_v16.py  (then select {dest})")
    print(f"\nTo serve time series:")
    print(f"  python serve_timeseries.py --folder {dest / 'predictions'}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Adapt coworker results to tuning-viz format (MAPE/MSE/CRPS)")
