"""
V12 Unified Plot: Scatter / Boxplot / Violin in one HTML.
- Multi-select checkbox filters for all parameter dropdowns
- Dynamic data series with [+] / [x]
- Violin/Box offset toggle (group vs overlay)
- Boxmean toggle for box plots
...
def plot_unified_v12(data: pd.DataFrame, metric_column: str, output_dir: Optional[str] = None) -> Optional[str]:
...
<div class="controls">
    ...
    <div class="filter-bar">
        ...
        <button id="offsetBtn" class="btn" style="display:none;" onclick="toggleOffset()">Offset: ON</button>
...
function setPlotType(type) {{
    ...
    document.getElementById('offsetBtn').style.display = (type === 'scatter') ? 'none' : '';
    rebuild();
}}
...
function rebuild() {{
    ...
    var vMode = showOffset ? 'group' : 'overlay';
