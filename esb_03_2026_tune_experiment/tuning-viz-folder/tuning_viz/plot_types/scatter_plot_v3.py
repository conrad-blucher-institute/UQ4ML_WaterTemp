"""
V3 Scatter plots: categorical x-axes, combined + per-cycle views.
Color = activation, symbol = leadtime (toggleable in legend).
"""
...
def _build_scatter_figure(data: pd.DataFrame, metric_column: str, title_suffix: str = "") -> go.Figure:
    """Build a single scatter figure with multiple x-axis rows."""
    clean = data.copy()
...
def plot_scatter_v3(data: pd.DataFrame, metric_column: str, output_dir: Optional[str] = None) -> List[go.Figure]:
    """
