...
# -*- coding: utf-8 -*-
"""
Spaghetti Graph Runner - Plotly Version
...
def calculate_std_dev_metrics(pred_columns, df_combined):
    """
    Calculate standard deviation metrics from all model predictions.
    ...
    Returns:
    --------
    dict with keys: 'mean', 'stdev', 'below2std', 'above2std'
    """
    
    # Extract predictions into a subset dataframe
    pred_data = df_combined[pred_columns]
    
    # Calculate metrics
    mean = pred_data.mean(axis=1)
    stdev = pred_data.std(axis=1)
    
    below2std = mean - (2 * stdev)
    above2std = mean + (2 * stdev)
...
# Toggle to show standard deviation bands (±2 std dev)
show_std_dev = True
...
def create_spaghetti_plot(df_combined, leadTime, cycle, architecture, obsVsPred):
    ...
    # Calculate standard deviation metrics if enabled
    if show_std_dev:
        std_metrics = calculate_std_dev_metrics(pred_columns, df_combined)
        
        # Add lower bound of ±2 std dev band
        fig.add_trace(go.Scatter(
            x=date_time,
            y=std_metrics['below2std'],
            mode='lines',
            name='±2 Std Dev Band',
            line=dict(color='rgba(100, 100, 100, 0)', width=0),
            showlegend=True,
            hovertemplate='<b>Std Dev Lower</b><br>Date: %{x}<br>Temp: %{y:.2f}°C<extra></extra>'
        ))
        
        # Add upper bound with fill to create band
        fig.add_trace(go.Scatter(
            x=date_time,
            y=std_metrics['above2std'],
            mode='lines',
            name='±2 Std Dev Band',
            line=dict(color='#A8E6A1', width=0),
            fill='tonexty',
            fillcolor='#A8E6A1',
            showlegend=False,
            hovertemplate='<b>Std Dev Upper</b><br>Date: %{x}<br>Temp: %{y:.2f}°C<extra></extra>'
        ))
