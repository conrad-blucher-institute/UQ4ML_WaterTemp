"""
Run V14 visualizations for tuning results.

Usage:
    python run_v14.py                       # GUI folder picker
    python run_v14.py --test-iterations 3   # duplicate data 3x for testing iteration plot

V14 changes:
    - Explorer: "Send to Time Series" button via rectangle/lasso select + localStorage
    - Time Series: Refresh button, no zoom sync, always draws observation line
"""
...
from tuning_viz.plot_types.unified_plot_v14 import plot_unified_v14
...
from tuning_viz.plot_types.timeseries_compare_v14 import plot_timeseries_compare_v14
...
def run_v14(data, metric_column, output_dir, results_folder=None):
    print(f"\n{'='*60}")
    print(f"V14 Visualizations: {output_dir}")
    print(f"{'='*60}")
    ...
    plot_unified_v14(data, metric_column, output_dir=output_dir)
    ...
    if results_folder:
        ...
        if Path(pred_dir).exists() and (Path(pred_dir) / '_index.csv').exists():
            ...
            plot_timeseries_compare_v14(pred_dir, output_dir=output_dir)
        else:
            print("\n--- Time Series Comparison: SKIPPED (no predictions/ folder, run generate_predictions.py first) ---")

    print(f"\n{'='*60}")
    print(f"All v14 plots saved to: {output_dir}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run V14 tuning visualizations")
    parser.add_argument(
    ...
    for csv_path in csv_paths:
        ...
        run_v14(data, metric_column, output_dir, results_folder=str(folder))
