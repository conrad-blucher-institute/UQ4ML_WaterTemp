"""
Run V17 visualizations for tuning results.

Usage:
    python run_v17.py                       # GUI folder picker
    python run_v17.py --test-iterations 3   # duplicate data 3x for testing iteration plot

V17 changes:
    - Time Series: metric checkboxes (multi-select) instead of dropdown,
      metric values shown in trace labels, "Metric Values" toggle button
    - Explorer: hover tooltip now shows ALL metric values (train, test, val, 2021),
      "All Metrics in Hover" toggle button
"""
...
from tuning_viz.plot_types.timeseries_compare_v17 import plot_timeseries_compare_v17
...
def run_v17(data, metric_column, output_dir, results_folder=None):
    print(f"\n{'='*60}")
    print(f"V17 Visualizations: {output_dir}")
    print(f"{'='*60}")
    ...
    if results_folder:
        ...
        if Path(pred_dir).exists() and (Path(pred_dir) / '_index.csv').exists():
            print("\n--- Time Series Comparison (v17) ---")
            plot_timeseries_compare_v17(pred_dir, output_dir=output_dir)
        else:
            print("\n--- Time Series Comparison: SKIPPED (no predictions/ folder, run generate_predictions.py first) ---")

    print(f"\n{'='*60}")
    print(f"All v17 plots saved to: {output_dir}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run V17 tuning visualizations")
    parser.add_argument(
    ...
    for csv_path in csv_paths:
        ...
        run_v17(data, metric_column, output_dir, results_folder=str(folder))
