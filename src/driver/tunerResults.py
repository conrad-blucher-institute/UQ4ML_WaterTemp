...
num_mae_values = len(cycle_list) * runs * len(hours_back) * max_trials # variable to store range of total number of 'val_mae' values
...
file2.write('Cycle:' + ',' + 'Iteration:' + ',' + 'Trial Number:' + ',' + objective +' Score:' + ',' + 'layer_units_' + 'num' + ','
    ...
    for h in hours_back:
        ...
        for r in range(runs):
            ...
            for m in range(max_trials):
                ...
                location = f"{dataset}/Iter_{str(r+1)}_{leadtime}h_Cycle_{c}/results/trial_{str(m)}/trial.json"
            ...
            if allTrials == True:
                for trial in bestObjective:
                    ...
                    newLocation = location = f"{dataset}/Iter_{str(r+1)}_{leadtime}h_Cycle_{c}/results/trial_{str(trial)}/trial.json"
                    ...
                    file2.write(str(c) + ',' +  str(r + 1) + ',' + str(trial) + ',' + str(best_valObjective) + ',' 
                ...
                # Grabs the location of the min trial
                newLocation = dataset +"/" + str(leadtime) +'h_LeadTime_Cycle_'  + str(c+1) + '_MLPTuner/' + str(h) + '_hours_back_ iteration_' + str(r+1) + '/trial_' + str(min(bestObjective, key = bestObjective.get)) + '/trial.json'
                ...
                file2.write(str(c) + ',' +  str(r + 1) + ',' + str(min(bestObjective, key = bestObjective.get)) + ',' + str(best_valObjective) + ',' 
